...
cat input.csv | while read line ; do
	...
	echo "---------- $key === $value --------"
	curl -vsw "\nHTTP-CODE=%{http_code}" -H 'Content-Type: text/plain' -X PUT http://127.0.0.1:8080/redis/api/v1/key/$key -d "$value" 2>&1 | grep -v -E '^(\*|>|< HTTP|< X-Pow|< Content|< ETag|< Date|< Connection|\{)' | grep -v 'data not shown'
	printf "\n"
