module.exports = function () {
    ...
    Redis.prototype._all = function(logMsg, command, next) {
        ...
        var transaction = {
            logMsg    : logMsg,
            nbServers : this.servers.length,
            majority  : Math.floor(this.servers.length /2) +1,
            success   : 0,
            fail      : 0,
            done      : false
        };
        ...
        this.servers.forEach(function (server) {
            process.nextTick(function () {
                command(server, function (err, values) {
                    ...
                    if (transaction.success === transaction.majority) {
                        transaction.done = true;
                        next(err, values, server);
                    } else if (transaction.fail === transaction.majority) {
                        transaction.done = true;
                        next(err, values, server);
                    } else if (transaction.success + transaction.fail >= transaction.nbServers) {
                        $this.logger.log('debug', "redis(%s)::%s|err=%j|transaction=%j", $this.config.redis_urls, logMsg, err, transaction, $this.meta);
                        if (!transaction.done) {
                            transaction.done = true;
                            next(new Error("redis::" + logMsg + "|FAILED"), values, server);
                        }
                    }
                });
            });
