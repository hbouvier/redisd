module.exports = function () {
    ...
    Redis.prototype.set = function (key, value, next) {
        ...
        this._majority('set(key=' + key + ', value=' + value + ')', function (server, callback) {
            ...
            var channel = 'redis:set',
                object = {key:key, value:value};
             $this._majority('publish(channel=' + channel +', message=' + JSON.stringify(object) + ')', function (server, callback) {
                 object.server = server.url;
                 var message = JSON.stringify(object);
                console.log('publish(channel=' + channel +', message=' +message);
                server.redis.publish(channel, message);
            });
            next(err, result, server);
        });
    };
    ...
    Redis.prototype.subscribe = function (channel) {
        this._majority('subscribe(channel=' + channel + ')', function (server, callback) {
